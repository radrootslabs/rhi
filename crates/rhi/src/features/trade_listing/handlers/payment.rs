...
pub async fn handle_job_request_trade_payment(
    ...
    let invoice_evt = nostr_fetch_event_by_id(client.clone(), &req.invoice_result_event_id)
        .await
        .map_err(|_| JobRequestPaymentError::FetchReference(req.invoice_result_event_id.clone()))?;
    if invoice_evt.kind != nostr::event::Kind::Custom(KIND_TRADE_LISTING_INVOICE_RES) {
